...
public class Matrix {
    ...
    static Matrix subtract(Matrix a, Matrix b) {
        ...
        // Return a new MyKled.Matrix a-b
    ...
    void randomize() {
        for (int i = 0; i < this.rows; i++) {
            for (int j = 0; j < this.cols; j++) {
                this.data[i][j] = Math.random() > 0.5? Math.random() * 2 - 1 : Math.random() * -1 ;
            }
        }
    }
    ...
    static Matrix multiply(Matrix a, Matrix b) {
        // MyKled.Matrix product
        if (a.cols != b.rows) {
            System.out.println("multiply: Columns of A must match rows of B. Input:" + a + " * " + b);
